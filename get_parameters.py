...
# 3. Tire degradation curves by compound and tire age
#------------------------------------------------------------------
...
def get_max_stint_length():
    ...
    max_stints = df_stints.groupby('compound')['stint_length'].max().reset_index()
    max_stints.to_csv('data/parameter/max_stint_lengths.csv', index=False)
...
def get_degradation_stats():
    '''
    Generates tire degradation curves from lap and stint data.
    '''
    ...
    # Removes data with sample size < 5
    degradation = degradation[degradation['sample_size'] >= 5]
    # Applies smoothing and normalization
    degradation = smooth_and_normalize(degradation)

    degradation.to_csv('data/parameter/degradation_curve.csv', index=False)
    return degradation